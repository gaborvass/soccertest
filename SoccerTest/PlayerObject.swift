...
class PlayerObject : NSObject{
    ...
    */
    private func pass(to: PlayerObject) -> PlayerActionResult {
        var retValue: PlayerActionResult = PlayerActionResult.BALL_LOST
        let random : Int = Int(arc4random_uniform(100))
        if random >= 100 - self.currentPassingSkill {
            retValue = PlayerActionResult.BALL_KEPT
        }
        return retValue
    }
    ...
    **/
    func move() -> PlayerActionResult{
        ...
        default: result = self.pass(self.teamDelegate!.selectTeamMemberToPassTo())
        }
